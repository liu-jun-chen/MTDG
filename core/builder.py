...
def get_synlidar():
    ...
    dataset_config = configs.src_dataset2
    dataset = SynLiDAR(root=dataset_config.root,
                       num_points=dataset_config.num_points,
                       voxel_size=dataset_config.voxel_size,
...
def get_poss():
    from core.datasets.semantic_poss import SemanticPOSS
    dataset_config = configs.src_dataset3
    dataset = SemanticPOSS(root=dataset_config.root,
                       num_points=dataset_config.num_points,
                       voxel_size=dataset_config.voxel_size,
                       src=configs.tgt_dataset.name)
    return dataset['train']
...
def make_dataset() -> Dataset:
    ...
    src_dataset_kitti = get_kitti(phase='train')
    src_dataset_synlidar = get_synlidar()
    src_dataset_poss = get_poss()
    ...
    dataset = {}
    dataset['train_kitti'] = src_dataset_kitti
    dataset['train_synlidar'] = src_dataset_synlidar
    dataset['train_poss'] = src_dataset_poss
